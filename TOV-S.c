...
#define FILE_NAME "TOVnotS_data.dat"
...
void insertRecord(FILE *file, int key, const char *data) {
    ...
    // Prepare the new record with metadata
    ...
    while (readBlock(file, blockNumber, &block) == 1) {
        if (block.used_size + sizeof(int) * 2 + data_size <= BLOCK_SIZE) {
            ...
            while (offset < block.used_size) {
                ...
                memcpy(&current_key, &block.data[offset], sizeof(int));
                memcpy(&current_data_size, &block.data[offset + sizeof(int)], sizeof(int));

                // If the new key is less, insert before this record
                if (key < current_key) {
                    int move_size = block.used_size - offset;
                    memmove(&block.data[offset + sizeof(int) * 2 + data_size], &block.data[offset], move_size);

                    // Insert the new record at the current offset
                    memcpy(&block.data[offset], &new_record.key, sizeof(int));
                    memcpy(&block.data[offset + sizeof(int)], &new_record.data_size, sizeof(int));
                    memcpy(&block.data[offset + sizeof(int) * 2], new_record.data, data_size);
                ...
                offset += sizeof(int) * 2 + current_data_size;
            }

            // If it belongs at the end of the block
            if (!inserted) {
    ...
    if (!inserted) {
        ...
        writeBlock(file, blockNumber, &new_block);
    }
...
void deleteRecord(FILE *file, int key) {
    ...
    while (readBlock(file, blockNumber, &block) == 1) {
        ...
        while (offset < block.used_size) {
            ...
            if (record_key == key) {
                int move_size = block.used_size - (offset + sizeof(int) * 2 + data_size);
                memmove(&block.data[offset], &block.data[offset + sizeof(int) * 2 + data_size], move_size);
                block.used_size -= sizeof(int) * 2 + data_size;
                writeBlock(file, blockNumber, &block);
